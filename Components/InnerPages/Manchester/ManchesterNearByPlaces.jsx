...
export default function ManchesterNearByPlaces() {
  /********************* states *************** */
  ...
  const { data: nearbyRestaurantsData } = useQuery({
    queryKey: ["restaurantsNearby", "Manchester"],
    queryFn: () => NearbyRestaurant("Manchester"),
  });
        ...
        <div className="container">
          <div className="row">
            <div className="explore_section section_title m">
              <h2 className="mb-0">Popular Locations Around Manchester</h2>
              <p>
                Places to eat and places to visit are located near Manchester's
                iconic hotel area, making them easily accessible during your
                stay and local travel planning.
              </p>
            </div>
          </div>
            ...
            <div className="row ">
              ...
              >
                {nearbyPlaceslist?.map((item, i) => {
                  ...
                  const title =
                    item?.displayName?.text || item?.displayName || "Place";
                  const placeType =
                    getPlaceTypeFromTypes(item?.types) || "restaurant";
                  const slug = placeId
                    ? getPlaceDetailPath(placeType, title, placeId)
                    : "#";
                  return (
                    <SwiperSlide key={i}>
