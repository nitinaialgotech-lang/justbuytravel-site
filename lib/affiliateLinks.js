...
export function buildAffiliateLinks(opts) {
  ...
  const bookingUrl = `https://flights.booking.com/flights/${dep}.AIRPORT-${arr}.AIRPORT/?adults=${adults}&ca_source=flights_index_sb&cabinClass=${cabin}&depart=${outD}&from=${dep}.AIRPORT&fromCountry=IN&fromLocationName=&sort=BEST&to=${arr}.AIRPORT&toCountry=IN&toLocationName=&travelPurpose=leisure&type=${isRound ? "ROUNDTRIP" : "ONEWAY"}${bookingChildren}`;
  const bookingUrlWithReturn = retD
    ? `https://flights.booking.com/flights/${dep}.AIRPORT-${arr}.AIRPORT/?adults=${adults}&ca_source=flights_index_sb&cabinClass=${cabin}&depart=${outD}&return=${retD}&from=${dep}.AIRPORT&fromCountry=IN&fromLocationName=&sort=BEST&to=${arr}.AIRPORT&toCountry=IN&toLocationName=&travelPurpose=leisure&type=ROUNDTRIP${bookingChildren}`
    : bookingUrl;
  links.push({
    name: "Booking.com",
